...
function kickBackgroundSync() {
  if (typeof window !== "undefined" || globalBoot.__mawinKickSync__) return;
  globalBoot.__mawinKickSync__ = true;
  void import("./wallet-server")
    .then((m) => m.startLotterySyncLoop())
    .catch((err) => console.error("[mawin] lottery-loop", err));
  void import("./football-feed")
    .then((m) => m.startFootballSyncLoop())
    .catch((err) => console.error("[mawin] football-loop", err));
}
