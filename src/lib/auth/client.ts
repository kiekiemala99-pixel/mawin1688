...
/** Pull the Better Auth session token out of a sign-in / sign-up response. */
export function captureAuthToken(ctx: { response?: Response; data?: unknown }) {
  const header = ctx.response?.headers.get("set-auth-token");
  const data = ctx.data as { token?: string; session?: { token?: string } } | null | undefined;
  const token = header || data?.token || data?.session?.token || null;
